// utils/filtering.ts (or inline in each page's <script setup>)
export type Filters = {
  q: string
  city: string | null
  status: string | null
...
export function applyFilters(items: any[], f: Filters) {
  ...
  switch (f.sort) {
    case 'new': out.sort((a, b) => new Date(b?.created_at ?? 0).getTime() - new Date(a?.created_at ?? 0).getTime()); break
    case 'old': out.sort((a, b) => new Date(a?.created_at ?? 0).getTime() - new Date(b?.created_at ?? 0).getTime()); break

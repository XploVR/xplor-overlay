...
export function useChat() {
  ...
  /**
   * Load conversations I can see (via conv RLS), then fetch my participant rows
   * for last_read_at. This avoids cross-table selects that can trigger recursive policies.
   */
  async function loadConversations() {
    ...
    // 1) Conversations visible to me (conv_select policy should allow SELECT)
    ...
    if (cErr) {
      loading.value = false
      lastError.value = cErr.message
      throw new Error(cErr.message)
    }

    conversations.value = convs ?? []

    // 2) My participant rows for those conversations (simple, non-recursive)
    const ids = conversations.value.map(c => c.id)
    for (const c of conversations.value) lastReadByConv[c.id] = null
    ...
    // 3) Prime unread counts
    await Promise.all(conversations.value.map(c => refreshUnreadFor(c.id)))

    // 4) Realtime inbox watcher
    attachInboxRealtime()

    loading.value = false
  }

  /** Global realtime for any new message */
  ...
  /** Open one conversation, load history, attach per-convo realtime, mark read */
  ...
  /** Mark as read (upsert participant row) */
  async function markConversationRead(conversationId: string) {
    ...
    const { error } = await supabase
      .from('conversation_participants')
      .upsert(
        { conversation_id: conversationId, user_id: myUserId.value, last_read_at: now },
  ...
  /** Server-count unread (excludes my messages, respects last_read_at) */
  ...
  /** Create a conversation, join myself, open it */
  async function startConversation(kind: 'dm' | 'group' = 'group', title = 'New Chat') {
    requireAuth()
    // IMPORTANT: include owner_id to satisfy RLS insert & immediate select
    const { data, error } = await supabase
      .from('conversations')
      .insert({
    kind,
    title,
    owner_id: myUserId.value,   // <-- keep if you added owner_id
    created_by: myUserId.value  // <-- required by your FK
  })
      .select('id, kind, title, created_at')
      .single()
    ...
    const { error: pErr } = await supabase
      .from('conversation_participants')
      .upsert(
        { conversation_id: data.id, user_id: myUserId.value, last_read_at: now },
        { onConflict: 'conversation_id,user_id' }
      )
    if (pErr) {
      lastError.value = pErr.message
  ...
  onMounted(() => {
    // session is reactive via useSupabaseUser()
  })
